if __name__ == "__main__":
    ...
    with get_openai_callback() as cb:
        from llm_medicaid_eligibility.core.html_to_python_clean_example import (
            url_to_python,
        )
    ...
    {python_snippets_joined}""",
            f"Ensure that the program implements all {len(category_urls)} Medicaid programs. Re-write the code: ",
        ]
        ...
        for prompt in prompts:
            ...
            chain_with_history.get_session_history("session").messages[
                -2
            ].pretty_print()
            output.pretty_print()
    print(cb)
